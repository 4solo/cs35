...
/* threadFunction */
long part;
float finalcolor[width][height][3];
scene_t scene;
int nthreads;
void* ThreadFunction(void *arg)
{
   ...
   if ((tID+1)==(long)nthreads)
     end=width;
   for( long px=start; px<end; ++px )
    {
        ...
        for( long py=0; py<height; ++py )
        {
             ...
             */
            finalcolor[px][py][0]=scaled_color[0];
	    finalcolor[px][py][1]=scaled_color[1];
	    finalcolor[px][py][2]=scaled_color[2];      
        }
    }
  pthread_exit(0); 
}
...
main( int argc, char **argv )
{
    ...
    for( int px=0; px<width; px++ )
    {
        ...
        {
           printf( "%.0f %.0f %.0f\n",
		    finalcolor[px][py][0], finalcolor[px][py][1], finalcolor[px][py][2]);
	}
	printf("\n");
    }
    free(threadID);
